...
*/
#ifndef DIGITAL_RAIN_ANIMATION_H
#define DIGITAL_RAIN_ANIMATION_H
...
// Adapter for the original Info Orbs ScreenManager drawing API.
class MatrixDisplay {
public:
    explicit MatrixDisplay(ScreenManager &manager) : m_manager(manager) {}
    int width() const { return SCREEN_SIZE; }
    int height() const { return SCREEN_SIZE; }
    static uint16_t color565(uint8_t r, uint8_t g, uint8_t b) {
        return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
    }
    void fillRect(int x, int y, int w, int h, uint16_t color) { m_manager.fillRect(x, y, w, h, color); }
    void setTextColor(uint16_t fg, uint16_t bg) { m_manager.setLegacyTextColor(fg, bg); }
    void setCursor(int x, int y) { m_x = x; m_y = y; }
    void setTextSize(uint8_t size) { m_size = size; m_manager.setLegacyTextSize(size); }
    void print(const String &text) {
        m_manager.setLegacyTextFont(1);
        m_manager.setLegacyTextDatum(TL_DATUM);
        m_manager.drawLegacyString(text, m_x, m_y, 1);
        m_x += text.length() * 6 * m_size;
    }
...
public:
    ...
    // initialization
    void init(MatrixDisplay *gfx, bool biggerText = false, bool alphabetOnly = false) {
        _gfx = gfx;
        _animMode = MATRIX;
        line_len_min = 3;
        line_len_max = 20;
